var db = function (obj) {
	...
	///待整理
	/*
	this.select = function (tbl_name,col_names,row_num) {
		dataBase.transaction(
			function (tx) {
				var row_names = typeof(col_names) == 'undefined' ? '*' : col_names,
					limit = typeof(row_num) == 'undefined' ? '' : ' limit ' + row_num,
					sql = 'select ' + row_names + ' from ' + tbl_name + limit;
				tx.executeSql(
					sql,
					[],
					function (tx,rst) {
						console.log('[Select]Success!\n[%s rows return]',rst.rows.length);
						//暂用全局变量row接收结果集
						window.all_data = rst;
					},
					function (tx,err) {
						console.log('[Select_error]%s \n[%s]' ,err.message, sql);
					}
				);
			}
		);
